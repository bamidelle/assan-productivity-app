...
def show_login():
    st.markdown("""
        <div style='text-align: center; padding: 4rem 2rem;'>
            ...
            <p style='font-size: 16px; color: #C9C9D1;'>32 Powerful Features • AI-Powered • Team Collaboration • Time Tracking</p>
        </div>
...
def show_main_app():
    ...
    with st.sidebar:
        ...
        if st.session_state.clocked_in:
            ...
            st.markdown(f"""
            <div style='background: rgba(72, 187, 120, 0.2); padding: 1rem; border-radius: 10px; margin: 1rem 0;'>
                <p style='margin: 0; color: #48BB78; font-weight: 600;'>⏰ Clocked In</p>
                <p style='margin: 0; color: #FFFFFF; font-size: 14px;'>{round(elapsed, 1)} minutes</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        if st.button("🚪 Logout", use_container_width=True):
            if st.session_state.clocked_in:
                st.warning("Please clock out before logging out!")
            else:
...
def show_add_task():
    ...
    if not pending.empty:
        ...
        if not st.session_state.clocked_in:
            ...
            selected_task = st.selectbox("Select task to clock in:", list(task_options.keys()))
            ...
            if selected_task:
                ...
                with col2:
                    if st.button("🟢 Clock In", key="clock_in_btn", use_container_width=True):
                        clock_in_task(task_id, task['task'])
                        st.rerun()
        else:
            current_task = st.session_state.df_tasks[st.session_state.df_tasks["id"] == st.session_state.current_task_id].iloc[0]
            elapsed = (datetime.now() - st.session_state.clock_in_time).total_seconds() / 60
            
            st.markdown(f"""
            <div style='background: rgba(72, 187, 120, 0.1); padding: 1.5rem; border-radius: 12px; margin: 1rem 0;'>
                <h3 style='color: #48BB78; margin-top: 0;'>⏰ Currently Clocked In</h3>
                <p><strong>Task:</strong> {current_task['task']}</p>
                <p><strong>Clock In Time:</strong> {st.session_state.clock_in_time.strftime('%I:%M:%S %p')}</p>
                <p><strong>Elapsed Time:</strong> {round(elapsed, 2)} minutes</p>
            </div>
            """, unsafe_allow_html=True)
        ...
        st.info("No pending tasks available for time tracking. Add a task first!")
...
def show_timesheet():
    ...
    if st.session_state.df_timesheet.empty:
        st.info("No time entries yet. Clock in on a task to start tracking!")
        return
    
    user_timesheet = st.session_state.df_timesheet[
        st.session_state.df_timesheet["username"] == st.session_state.current_user
    ].copy()
    
    if user_timesheet.empty:
        st.info("You haven't clocked any time yet")
        return
    
    st.markdown("### 📊 Time Summary")
    
    total_minutes = user_timesheet["duration_minutes"].sum()
    total_hours = total_minutes / 60
    
    col1, col2, col3 = st.columns(3)
    col1.metric("Total Entries", len(user_timesheet))
    col2.metric("Total Hours", f"{total_hours:.2f}h")
    col3.metric("Total Minutes", f"{total_minutes:.0f}m")
    
    st.markdown("<div class='section-divider'></div>", unsafe_allow_html=True)
    
    st.markdown("### 📝 Time Entries")
    
    for _, entry in user_timesheet.sort_values("clock_in", ascending=False).iterrows():
        st.markdown(f"""
        <div class='card'>
            <p><strong>Task:</strong> {entry['task_name']}</p>
            <p><strong>Clock In:</strong> {entry['clock_in'].strftime('%Y-%m-%d %I:%M:%S %p')}</p>
            <p><strong>Clock Out:</strong> {entry['clock_out'].strftime('%Y-%m-%d %I:%M:%S %p')}</p>
            <p><strong>Duration:</strong> {entry['duration_minutes']:.2f} minutes ({entry['duration_minutes']/60:.2f} hours)</p>
        </div>
        """, unsafe_allow_html=True)
...
# Firebase Studio Dark Theme + Clock In/Out Feature
# ==============================================
...
<style>
    ...
    * {
    ...
    .clock-out-btn {
        ...
        font-size: 18px !important;
        font-weight: 700 !important;
        box-shadow: 0 4px 20px rgba(245, 101, 101, 0.4) !important;
    }
...
def clock_in_task(task_id, task_name):
    ...
    st.success(f"⏰ Clocked in at {st.session_state.clock_in_time.strftime('%H:%M:%S')}")

def clock_out_task():
    if st.session_state.clocked_in and st.session_state.clock_in_time:
        ...
        st.session_state.df_timesheet = pd.concat([st.session_state.df_timesheet, new_timesheet], ignore_index=True)
        ...
        st.success(f"✅ Clocked out! Duration: {round(duration, 2)} minutes")
